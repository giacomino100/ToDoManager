...
module.exports.assignAutomatically = function assignAutomatically (req, res, next) { /** DA CAPIRE */
...
module.exports.removeUser = function removeUser (req, res, next) {
  Assignments.removeUser(req.params.taskId, req.params.userId, req.user)
    ...
    .catch(function (response) {
      if(response == 404){
        utils.writeJson(res, { errors: [{"param" : "Server", "err" : "the task does not exist"}], }, 404);
      } else if(response == 403){
        utils.writeJson(res, { errors: [{"param" : "Server", "err" : "the owner is not the owner of the task"}], }, 403);
      } else{
        utils.writeJson(res, {errors: [{ 'param': 'Server', 'msg': response }],}, 500);
      }
    });
