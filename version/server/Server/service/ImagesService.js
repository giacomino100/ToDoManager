...
exports.addImage = function(taskId, file, owner) {
    return new Promise((resolve, reject) => {
        ...
        db.all(sql1, [taskId], (err, rows) => {
            if (err)
                reject(err);
            else if (rows.length === 0) // task does not exist
                reject(404);
            else if(owner != rows[0].owner) { //the user is not the owner of the task
                reject(403);
            }else {
                var nameFile = file.filename; 
                var nameWithoutExtension = nameFile.substring(0, nameFile.lastIndexOf(".") );
                var extension = nameFile.substring(nameFile.lastIndexOf(".")).replace('.', ''); //We take the extension to check it later
                ...
                //If until this point all work, we can insert image in db
                ...
                db.run(sql, [nameWithoutExtension], function(err) {
                    if (err) {
                        ...
                        const sql1 = 'INSERT INTO taskImages(taskId, imageId) VALUES(?, ?)';
                        db.run(sql1, [taskId, imageId], function(err) {
                            if (err) {
                                reject(err);
                            } else {
                                resolve({imageId: imageId, taskId: taskId, name: nameWithoutExtension});
                            }
                        });
                    }
                });
 ...
 *
 **/
   exports.getFileImage = function(imageId, imageType, taskId, owner) {
    return new Promise((resolve, reject) => {
 ...
 *
 **/
   exports.deleteImage = function(taskId, imageId, owner) {
    return new Promise((resolve, reject) => {
  ...
  /**
   * Retrieve the image associated to the task with ID imgId
   * This operation return the image associated to the task with a specified img id.\"
   *
   * taskId Long ID of the user
   * imageId Long The id of the requested image
   * returns Image
   **/
