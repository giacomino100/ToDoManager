...
const Main = () => {
  ...
  const filters = [
    { label: 'All', id: 'all', filterFn: () => true },
    { label: 'Important', id: 'important', filterFn: t => t.important },
    { label: 'Today', id: 'today', filterFn: t => t.deadline && t.deadline.isToday() },
    { label: 'Next 7 Days', id: 'nextweek', filterFn: t => isNextWeek(t.deadline) },
    { label: 'Private', id: 'private', filterFn: t => t.private },
  ...
  return (
    ...
    <Container fluid>
      <Row>
        <Navigation onLogOut={handleLogOut} logIn={doLogIn} loggedIn={loggedIn}  user={user} />
      </Row>
      ...
      <Switch>
        <Route path="/login">
          ...
          {!loggedIn ?  <LoginForm login={doLogIn}/> : <Redirect to="/list/all" />}
          </Row>
        </Route>
        <Route path={["/list/:filter"]}>
            <Row className="vh-100 below-nav">
              <TaskMgr taskList={taskList} filter={activeFilter} onDelete={deleteTask} onEdit={handleEdit} onCheck={handleCheck} onSelect={handleSelectFilter} loggedIn={loggedIn}></TaskMgr>
        ...
        <Route path="/">
          <Row className="vh-100 below-nav">
            {!loggedIn ?  <Redirect to="/list/public" />  : <LoginForm login={doLogIn} />}
          </Row>
        </Route>
      </Switch>
    </Container>
...
const TaskMgr = (props) => {
  ...
  const { taskList, filter, onDelete, onEdit, onCheck, onSelect, loggedIn } = props;
  ...
  return (
    <>
        <Col bg="light" className="d-block col-4" id="left-sidebar">
          <Filters items={filters} defaultActiveKey={activeFilter} onSelect={onSelect} loggedIn={loggedIn}/>
        </Col>
      <Col className="col-8">
        <h1 className="pb-3">Filter: <small className="text-muted">{activeFilter}</small></h1>
        <ContentList
          tasks={taskList}
          onDelete={onDelete} onEdit={onEdit} onCheck={onCheck} loggedIn={loggedIn} next={ne}
        />
      </Col>
    </>
  )
